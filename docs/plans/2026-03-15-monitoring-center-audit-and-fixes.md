...
2) **“实时连接”时完全停轮询，存在“假在线不刷新”风险**  
   - 位置：`frontend/src/features/monitoring/components/MonitoringView.tsx` → `useMonitoringV2({ enablePolling: !wsConnected && pageVisible })`  
   - 场景：WS 已连接但后端短期无推送/订阅失败/推送异常 → 页面长期不更新。  
   - 期望：WS 在线也保留低频轮询兜底（或基于 lastUpdate 超时触发 refetch）。  
   - 状态：✅ 已修复（页面可见即轮询：在线 5min / 离线 2min）  
...
- 前端 Jest：新增监控 API normalize/回退/权限分区相关单测（聚焦 `fetchMonitoringDataV2`）。  
- 后端 Go：新增 `/monitoring/dashboard/v2` 契约测试（验证字段命名、sections 全量、权限降级）。  
- 联调冒烟：前后端启动后打开 `/monitoring`，观察：初次拉取、WS 订阅、推送触发刷新、离线轮询兜底、导出按钮权限。  
...
5) **测试：补强 P2 覆盖（≤60s）**  
   - 前端：新增 `MonitoringView` 组件级测试（空态/权限受限/部分失败/错误分级），避免文案回归。  
