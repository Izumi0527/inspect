...
// InterfaceTrafficInterface 单台设备当前处于 UP 状态的物理接口（接口流量视图的选择项，逻辑口已剔除）。
...
// DeviceInterfaceTraffic 单台设备某个物理接口的上行/下行流量时序，Points 单位 Mbps。
// Interface 是实际采用的接口（请求未指定时取 Interfaces 首个），无可选接口时为空且 Points 为空。
