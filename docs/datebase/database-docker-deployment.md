...
```mermaid
graph TB
    A[企业级网络设备巡检系统] --> B[PostgreSQL + TimescaleDB<br/>端口:5433]
    A --> C[Redis<br/>端口:6380]
...
- SQL 初始化脚本：`database/timescaledb-init.sql`
...
```bash
# PostgreSQL
pg_isready -h localhost -p 5433 -U inspect_dev

# Redis
redis-cli -h localhost -p 6380 -a dev_redis_2024 ping
```
...
```bash
# PostgreSQL 备份
pg_dump -h localhost -p 5433 -U inspect_dev inspect_system_dev > backups/postgres_backup.sql

# Redis 备份
redis-cli -h localhost -p 6380 -a dev_redis_2024 --rdb backups/redis_backup.rdb
```
