...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("inspect-start-infra-pg-protocol-" + [Guid]::NewGuid().ToString("N"))
...
public static class ClosingServer
{
...
"@
    $closingServerPath = Join-Path $testRoot "bin/closing-server.exe"
    Add-Type -TypeDefinition $closingServerSource -OutputAssembly $closingServerPath -OutputType ConsoleApplication

    $postgresPort = New-FreeTcpPort
    $redisPort = New-FreeTcpPort
    $serverProcesses += Start-Process -FilePath $closingServerPath -ArgumentList @($postgresPort) -WindowStyle Hidden -PassThru
    $serverProcesses += Start-Process -FilePath $closingServerPath -ArgumentList @($redisPort) -WindowStyle Hidden -PassThru
    Start-Sleep -Milliseconds 300

    @"
DATABASE_URL=postgresql://inspect_dev:dev_password_2024@127.0.0.1:$postgresPort/inspect_system_dev
REDIS_URL=redis://:dev_redis_2024@127.0.0.1:$redisPort/0
...
"@
    ...
    if (-not $failedAsExpected) {
        throw "start-infra 应在 PostgreSQL 宿主机协议不可用时失败，但脚本返回成功。Output:`n$outputText"
    }
    if ($outputText -notmatch "PostgreSQL host protocol is not ready") {
        throw "start-infra 失败信息应指出 PostgreSQL 宿主机协议未就绪。Output:`n$outputText"
    }

    Write-Host "start-infra postgres protocol readiness test passed."
} finally {
