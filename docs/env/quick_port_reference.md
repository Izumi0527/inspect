...
| 服务 | 端口 | 协议 | 说明 |
|------|------|------|------|
| 前端应用 | **3000** | HTTP | Next.js（宿主机端口，可通过 FRONTEND_HOST_PORT 调整） |
| 后端 API | **8000** | HTTP/WebSocket | Go API（开发默认端口；Docker 可通过 BACKEND_HOST_PORT 覆盖） |
| Syslog 接收 | 5514 | TCP/UDP | 设备 Syslog 上报（UDP+TCP），用于日志中心与告警联动 |
| PostgreSQL | **15500** | TCP | TimescaleDB 数据库 |
| Redis | 26380 | TCP | 缓存服务 |
...
```powershell
# Windows
netstat -ano | findstr "3000 8000 5514 15500 26380"
