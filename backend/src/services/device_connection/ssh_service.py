...
from .types import SSHConfig, DeviceInfo
...
class SSHConnectionPool:
            ...
            for conn_key in to_remove:
                ...
                if conn_info and conn_info.get("connection"):
                    try:
                        ...
                        logger.debug("Closed idle SSH connection", key=conn_key)
                    except:
                        pass
    ...
    def get_connection(self, connection_info: Dict[str, Any]) -> Optional[ConnectHandler]:
        ...
        with self.lock:
            ...
            try:
                ...
                logger.debug("Created new SSH connection", key=conn_key)
    ...
    def _remove_oldest_connection(self):
        ...
        if conn_info and conn_info.get("connection"):
            try:
                conn_info["connection"].disconnect()
                logger.debug("Removed oldest SSH connection", key=oldest_key)
            except:
                pass
    
    def release_connection(self, connection: ConnectHandler):
        """释放连接（标记为可用）"""
        # 连接池模式下不立即关闭连接，只更新使用时间
        conn_key = None
        with self.lock:
        ...
        if conn_key:
            logger.debug("Released SSH connection", key=conn_key)
...
class SSHService(DeviceConnection):
    ...
    def __init__(self, max_connections: int = 10, idle_timeout: int = 300):
        ...
        self._active_connection: Optional[ConnectHandler] = None
    ...
    async def connect(self, device_info: Union[Dict[str, Any], DeviceInfo]) -> bool:
        ...
        try:
            if isinstance(device_info, dict):
                config = SSHConfig(
                    ...
                    device_type=self._get_netmiko_device_type(
                        device_info.get("vendor", "cisco"),
                        device_info.get("device_type", "switch")
                    )
                )
            else:
                config = SSHConfig(
            ...
            if not config.username or not config.password:
                self.logger.warning("SSH credentials incomplete")
                return False
            
            # 测试连接
            connection_info = await self.connect_device(device_info if isinstance(device_info, dict) else device_info.to_dict())
            
            if connection_info and connection_info.get("connected"):
                self._connected = True
                self._current_config = config
                self.logger.info("SSH connection established", host=config.host)
                return True
    ...
    def _prepare_connection_info(self, device_info: dict, timeout: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error("Failed to prepare SSH connection info", 
    ...
    def _sync_connect_device(self, connection_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("Failed to get version info", error=str(e))
            ...
            try:
                ...
                self.logger.warning("Failed to get interface info", error=str(e))
            
            return device_info
            
        except Exception as e:
            self.logger.error("SSH connection execution failed", 
    ...
    def _parse_version_info(self, version_output: str, device_type: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.warning("Failed to parse version info", error=str(e))
    ...
    def _count_interfaces(self, interface_output: str, device_type: str) -> int:
        ...
        try:
            ...
            self.logger.warning("Failed to count interfaces", error=str(e))
            return 0
    ...
    async def execute_single_command(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                result = await asyncio.get_event_loop().run_in_executor(
                    self.thread_executor,
                    self._sync_execute_command_with_pool,
                    connection_info,
                    command
                )
                ...
                self.logger.warning(f"SSH command failed on attempt {attempt + 1}", 
                                  ip=device_info.get("ip_address"),
                                  command=command,
                                  error=str(e))
                ...
                if attempt < self.max_retries - 1:
                    ...
                    self.logger.error("SSH command failed after all retries", 
                                    ip=device_info.get("ip_address"),
                                    command=command,
                                    error=str(e))
                    return None
                ...
                self.logger.error("SSH command execution failed", 
                                ip=device_info.get("ip_address"),
                                command=command,
                                error=str(e))
                return None
    ...
    def _sync_execute_command_with_pool(
        self, 
        ...
        try:
            ...
            self.logger.error("Sync SSH command execution failed", 
                            host=connection_info.get("host"),
                            command=command,
                            error=str(e))
    ...
    async def execute_batch_commands(
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                self.thread_executor,
                self._sync_execute_batch_commands,
                connection_info,
                commands
            )
            ...
            self.logger.error("SSH batch command execution failed", 
                            ip=device_info.get("ip_address"),
                            commands=commands,
                            error=str(e))
            return {cmd: None for cmd in commands}
    
    def _sync_execute_batch_commands(
        self,
        connection_info: Dict[str, Any],
        commands: List[str]
    ) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            for command in commands:
                try:
                    ...
                    self.logger.warning("Single command in batch failed", 
            ...
            self.logger.error("Sync batch SSH commands failed", 
                            host=connection_info.get("host"),
                            commands=commands,
                            error=str(e))
            return {cmd: None for cmd in commands}
    ...
    async def test_connection_with_retry(self, device_info: dict, max_attempts: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                self.logger.warning(f"Connection test failed on attempt {attempt + 1}",
                                  ip=device_info.get("ip_address"),
                                  error=str(e))
    ...
    async def backup_configuration(
        ...
        try:
            ...
            # 执行配置备份命令
            config_output = await self.execute_single_command(device_info, config_command, timeout)
            
            if config_output:
                self.logger.info("Configuration backup successful", 
                               ip=device_info.get("ip_address"),
                               config_length=len(config_output))
                return config_output
            else:
                self.logger.warning("Configuration backup failed - no output", 
                                 ip=device_info.get("ip_address"))
                return None
                
        except Exception as e:
            self.logger.error("Configuration backup failed", 
                            ip=device_info.get("ip_address"),
                            error=str(e))
            return None
    
    def get_available_commands(self, vendor: str = "cisco", device_type: str = "switch") -> Dict[str, str]:
        """获取可用的命令列表"""
        netmiko_type = self._get_netmiko_device_type(vendor, device_type)
        return self.command_mapping.get(netmiko_type, self.command_mapping["cisco_ios"])
