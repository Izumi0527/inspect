...
router = APIRouter(prefix="/api/alerts", tags=["告警升级"])
...
async def get_escalation_rules(
    ...
    for rule in alert_escalation_service.escalation_rules.values():
        rules.append(EscalationRuleResponse(
            id=rule.id,
            name=rule.name,
...
@router.get("/escalation/status/{alert_id}", response_model=Optional[EscalationStatusResponse])
...
@router.post("/escalation/cancel/{alert_id}")
...
@router.post("/escalation/test/{alert_id}")
