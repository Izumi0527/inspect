...
class CacheService:
    ...
    PREFIXES = {
        ...
        "inspection_stats": "insp_stats",
        "inspection_trends": "insp_trends",
        "device_dist": "dev_dist",
        "problem_dist": "prob_dist"
    }
    ...
    DEFAULT_EXPIRE = {
        ...
        "device_status": 60,
        "device_metrics": 300,
        "inspection_stats": 180,
        "inspection_trends": 240,
        "device_dist": 300,
        "problem_dist": 240
    }
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if key_func:
                ...
                cache_key = f"{key_prefix}_{svc._hash_key(params_str)}"
            
            cached_result = await svc.redis.get(cache_key)
            if cached_result is not None:
                logger.debug("Cache hit", key=cache_key, function=func.__name__)
                return cached_result
            
            result = await func(*args, **kwargs)
            await svc.redis.set(cache_key, result, expire=expire)
