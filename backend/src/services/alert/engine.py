...
class AlertEngine:
    ...
    async def start(self):
        ...
        if self.escalation_service is None:
            try:
                from src.services.alert.escalation import alert_escalation_service
    ...
    def _check_condition(self, condition: RuleCondition, value: Any, threshold: Any) -> bool:
        """检查条件是否满足"""
        try:
            if condition in (RuleCondition.GREATER_THAN, RuleCondition.GT):
                return float(value) > float(threshold)
            elif condition in (RuleCondition.LESS_THAN, RuleCondition.LT):
                return float(value) < float(threshold)
            elif condition == RuleCondition.GTE:
                return float(value) >= float(threshold)
            elif condition == RuleCondition.LTE:
                return float(value) <= float(threshold)
            elif condition in (RuleCondition.EQUAL, RuleCondition.EQ):
                return value == threshold
            elif condition in (RuleCondition.NOT_EQUAL, RuleCondition.NEQ):
                return value != threshold
